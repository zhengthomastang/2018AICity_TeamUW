...
void proc_img_flr(char *datacfg, char *cfgfile, char *weightfile, char *inflrname, char *outflrname, char *outtxtname, float thresh, float hier_thresh, int stfrmcnt, int ndfrmcnt)
{
    ...
    for (frmcnt = stfrmcnt; frmcnt <= ndfrmcnt; ++frmcnt) {
        ...
        char output[128] = { 0 };
        sprintf(output, outflrname);
        strcat(output, outputnm);

        output_detections(frmcnt, outflrname, outtxtname, im, l.w*l.h*l.n, thresh, boxes, probs, masks, names, alphabet, l.classes);
...
void run_lp_det(char *datacfg, char *cfgfile, char *weightfile, char *intxtname, char *outtxtname, float thresh, float hier_thresh)
{
    list *options = read_data_cfg(datacfg);
    char *name_list = option_find_str(options, "names", "data/names.list");
    char **names = get_labels(name_list);
    ...
    FILE* intxt = fopen(intxtname, "r");
    char * line = NULL;
    size_t len = 0;
    ssize_t read;

    while ((read = getline(&line, &len, intxt)) != -1) {
        char *input = (char *) calloc(strlen(line),sizeof(char));
        strncpy(input,line,strlen(line)-1);
        image im = load_image_color(input,0,0);
        image sized = letterbox_image(im, net->w, net->h);
        layer l = net->layers[net->n-1];
        ...
        output_lp_detections(input, outtxtname, im, l.w*l.h*l.n, thresh, boxes, probs, masks, names, alphabet, l.classes);
